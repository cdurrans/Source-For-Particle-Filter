    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          vector<Particle> particles_main = pf.particles;
          int num_particles = particles_main.size();
          double highest_weight = -1.0;
          Particle best_particle;
          double weight_sum = 0.0;
          for (int i = 0; i < num_particles; ++i) {
            if (particles_main[i].weight > highest_weight) {
              highest_weight = particles_main[i].weight;
              best_particle = particles_main[i];
            }

            weight_sum += particles_main[i].weight;
          }
